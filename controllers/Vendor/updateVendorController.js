...
exports.updateProfile = async (req, res) => {
    try {
        ...
        if (req.files?.profileImage) {
            updates.profileImage = `/uploads/Vendor/profileImg/${req.files.profileImage[0].filename}`;
        }
        if (req.files?.cnicImage && sanitizedCnic) {
            updates.cnicImage = `/uploads/Vendor/cnicImg/${sanitizedCnic}/${req.files.cnicImage[0].filename}`;
        }
